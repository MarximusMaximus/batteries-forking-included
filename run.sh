...
(
    ...
    TEMP_RUN_EXEC="$1"
    if [ ! -f "$1" ]; then
        if [ "$(type "${TEMP_RUN_EXEC}" 2>&1 | grep "\(is a shell builtin\|not found\)" )" = "" ]; then
    ...
    if [ -f "${TEMP_RUN_EXEC}" ]; then
        ...
        shift
    elif [ "$(type "${TEMP_RUN_EXEC}" 2>&1 | grep "is a shell builtin" )" != "" ]; then
        # arg 1 is directly an shell builtin
        RUN_EXEC="${TEMP_RUN_EXEC}"
        RUN_ARGS=""
        shift
    fi
