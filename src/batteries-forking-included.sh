...
usage_text=$(cat<<EOF
Usage:
    batteries-forking-included.sh [flags|options]
    ...
    while true; do
        ...
        case "$1" in
            ...
            +d|--no-dev|--no-developer)
                log_ultradebug "$(get_my_real_basename)::parse_args::while;\t found no-dev arg"
                dev_mode=false
                ;;

            -D|--deploy|--deployment)
                log_ultradebug "$(get_my_real_basename)::parse_args::while;\t found deploy arg"
                deploy_mode=true
                ;;
            +D|--no-deploy|--no-deployment)
                log_ultradebug "$(get_my_real_basename)::parse_args::while;\t found no-deploy arg"
                deploy_mode=false
                ;;

            -p|--project-dir)
                log_ultradebug "$(get_my_real_basename)::parse_args::while;\t found project-dir arg"
                if [ -n "$2" ]; then
                    project_dir="$2"
                    log_ultradebug "$(get_my_real_basename)::parse_args::while;\t\t project_dir=%s" "${project_dir}"
                    shift
                else
                    usage
            ...
            -?*)    # positive flags
                ...
                while true; do
                    ...
                    case "${arg_char}" in
                        ...
                        d)
                            log_ultradebug "$(get_my_real_basename)::parse_args::while::while(-);\t found dev flag"
                            dev_mode=true;
                            ;;

                        D)
                            log_ultradebug "$(get_my_real_basename)::parse_args::while::while(-);\t found deploy flag"
                            deploy_mode=true;
                            ;;
            ...
            +?*)    # negative flags
                ...
                while true; do
                    ...
                    case "${arg_char}" in
                        ...
                        D)
                            log_ultradebug "$(get_my_real_basename)::parse_args::while::while(+);\t found no-deploy flag"
                            deploy_mode=false;
                            ;;
...
batteries_forking_included () {
    log_ultradebug "batteries_forking_included called with '%s'" "$*"
